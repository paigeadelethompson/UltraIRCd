...
{
  xmlChar *use_logging = xmlGetProp(node, (const xmlChar *)"use_logging");
  if (use_logging)
    ConfigLog.use_logging = xmlStrcmp(use_logging, (const xmlChar *)"yes") == 0;
  xmlFree(use_logging);
  ...
  for (xmlNodePtr child = node->children; child; child = child->next)
  {
    ...
    if (xmlStrcmp(child->name, (const xmlChar *)"file") == 0)
    {
      ...
      if (name && type)
      {
        unsigned int log_type = LOG_TYPE_IRCD;  // Default
        if (xmlStrcmp(type, (const xmlChar *)"user") == 0)
          log_type = LOG_TYPE_USER;
        else if (xmlStrcmp(type, (const xmlChar *)"operator") == 0)
          log_type = LOG_TYPE_OPER;
        else if (xmlStrcmp(type, (const xmlChar *)"xline") == 0)
          log_type = LOG_TYPE_XLINE;
        else if (xmlStrcmp(type, (const xmlChar *)"resv") == 0)
          log_type = LOG_TYPE_RESV;
        else if (xmlStrcmp(type, (const xmlChar *)"dline") == 0)
          log_type = LOG_TYPE_DLINE;
        else if (xmlStrcmp(type, (const xmlChar *)"kline") == 0)
          log_type = LOG_TYPE_KLINE;
        else if (xmlStrcmp(type, (const xmlChar *)"kill") == 0)
          log_type = LOG_TYPE_KILL;
        else if (xmlStrcmp(type, (const xmlChar *)"debug") == 0)
          log_type = LOG_TYPE_DEBUG;

        unsigned int size_value = size ? parse_size_spec((const char *)size) : 0;
        log_add(log_type, false, size_value, (const char *)name);
      }
